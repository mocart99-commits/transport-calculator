...
try:
    ...
    st.error("Грешка при API ключа в Secrets!")
    st.stop()
...
with c1:
    km_price = st.number_input("Цена без ДДС (€/км):", value=4.00, step=0.50, format="%.2f")
with c2:
    ...
    if customer_addr:
        try:
            ...
            if directions:
                ...
                # Показване на разстояние и цена на един ред
                res_col1, res_col2 = st.columns(2)
                res_col1.metric("Разстояние", f"{dist_km:.1f} км")
                res_col2.metric("Обща цена (без ДДС)", f"{total_cost:.2f} €")
                
                # Подготовка на съобщение за споделяне
                share_text = f"ГЕОТОН - Транспортна калкулация:\nАдрес: {customer_addr}\nРазстояние: {dist_km:.1f} км\nЦена: {total_cost:.2f} € без ДДС"
                msg_encoded = urllib.parse.quote(share_text)
                
                # Бутони за споделяне
                col_g, col_v, col_e = st.columns(3)
                with col_g:
                    g_link = f"https://www.google.com/maps/dir/?api=1&origin={lat},{lng}&destination={urllib.parse.quote(customer_addr)}&travelmode=driving"
                    st.markdown(f'<a href="{g_link}" target="_blank" class="share-btn google-btn">Google Maps</a>', unsafe_allow_html=True)
                with col_v:
                    v_link = f"viber://forward?text={msg_encoded}"
                    st.markdown(f'<a href="{v_link}" class="share-btn viber-btn">Viber</a>', unsafe_allow_html=True)
                with col_e:
                    e_link = f"mailto:?subject=Транспортна калкулация&body={msg_encoded}"
                    st.markdown(f'<a href="{e_link}" class="share-btn email-btn">Имейл</a>', unsafe_allow_html=True)
                ...
                m = folium.Map()
                # ФИКСИРАНО: Правилно затворени скоби за обхвата на картата
                sw = [min(p[0] for p in route_line), min(p[1] for p in route_line)]
                ne = [max(p[0] for p in route_line), max(p[1] for p in route_line)]
                m.fit_bounds([sw, ne])
                ...
                st.error("Маршрутът не е намерен.")
