...
# 2. ЗАГЛАВИЕ И ЛОГО
try:
    # Опитва се да зареди лого, ако го качиш в GitHub като logo.png
...
try:
    ...
    st.error("Грешка: Липсва Google Maps API Key!")
    st.stop()

gmaps = googlemaps.Client(key=API_KEY)

# 4. ВХОДНИ ДАННИ
st.markdown("### 📍 Детайли за доставката")

# Начална точка (Константа)
st.markdown("<p class='info-text'>Начална точка: Производствена база Геотон</p>", unsafe_allow_html=True)

# Полета за цена и адрес
...
if st.button("ИЗЧИСЛИ ТРАНСПОРТ"):
    if customer_addr:
        try:
            ...
            if not directions:
                st.error("Адресът не е намерен! Моля, опитайте отново.")
            else:
                res = directions[0]
                ...
                with res_col2:
                    st.metric("Обща цена (без ДДС)", f"{total_cost:.2f} €")
